...
Body: `{"magnet": "magnet:?xt=urn:...", "category": "movies"}` or `{"auto": "The Matrix", "category": "movies"}`
Use this to start a download.
If the user asks to "download X", you can just send `{"auto": "X"}` and Gorrent will automatically find and start the best match. You can optionally include a `category` (like movies, tvshows, anime) to automatically route the files to specific homelab folders.
